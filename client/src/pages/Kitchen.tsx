...
import { Bell, Check, ChefHat, Clock3, ExternalLink, Flame, PackageCheck, Volume2, VolumeX } from "lucide-react";
...
export default function Kitchen() {
  ...
  const visible = orders.data?.filter((order) => filter === "todos" || order.status === filter) || [];
  const counts = {
  ...
  return <div className="min-h-screen bg-[#141713] text-[#f3f1e9]">
    <header className="border-b border-white/10 bg-[#1d201c]"><div className="container flex min-h-[78px] flex-wrap items-center justify-between gap-4 py-4"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-[14px] bg-[#e8ff61] text-xl text-[#1d201c]">✦</div><div><div className="font-display text-lg font-black tracking-[-0.04em]">BOCA & BRASA <span className="text-[#e8ff61]">/ KDS</span></div><div className="mt-1 font-mono text-[9px] uppercase tracking-[0.2em] text-[#929b8d]">painel da cozinha · atualiza a cada 3s</div></div></div><div className="flex items-center gap-2"><button onClick={() => { setSoundOn((current) => !current); if (!soundOn) beep(); }} className="flex h-10 items-center gap-2 rounded-xl border border-white/10 px-3 text-xs font-bold text-[#c5cbbf] transition hover:bg-white/5">{soundOn ? <Volume2 size={15} /> : <VolumeX size={15} />}{soundOn ? "Som ativo" : "Som desligado"}</button><a href="/" className="flex h-10 items-center gap-2 rounded-xl border border-white/10 px-3 text-xs font-bold text-[#c5cbbf] transition hover:bg-white/5"><ExternalLink size={14} /> Cardápio</a></div></div></header>
    <main className="container pb-12 pt-8"><div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><div className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#8a9786]">fila de produção</div><h1 className="mt-2 font-display text-4xl font-black tracking-[-0.07em] sm:text-5xl">Pedidos ativos</h1></div><div className="flex gap-2 overflow-x-auto pb-1">{(["todos", "recebido", "preparando", "pronto"] as const).map((item) => <button key={item} onClick={() => setFilter(item)} className={`whitespace-nowrap rounded-xl border px-4 py-3 text-left transition ${filter === item ? "border-[#e8ff61] bg-[#e8ff61] text-[#1d201c]" : "border-white/10 bg-[#1d201c] text-[#aab3a5] hover:border-white/25"}`}><div className="font-mono text-[9px] uppercase tracking-[0.14em]">{item === "todos" ? "Todos" : statusLabel[item]}</div><div className="mt-1 font-display text-xl font-black">{counts[item]}</div></button>)}</div></div>
